class HotelsController < ApplicationController
  ...
  def index
    if params[:term]
      @hotels = Hotel.search_by_hotel_name(params[:term]).order(choose_people: :desc).order(id: :asc).page(params[:page1]).per Settings.paginate.hotels
      @hotels2 = Hotel.search_by_hotel_address(params[:term]).order(choose_people: :desc).order(id: :asc).page(params[:page2]).per Settings.paginate.hotels
      @keyword = params[:term]
    elsif params[:commit] == "フィルター" || params[:commit] == "完了"
      @hotels = Hotel.filter_by_service(params).order(choose_people: :desc).order(id: :asc).page(params[:page]).per Settings.paginate.hotels
    else
      @hotels = Hotel.all.order(choose_people: :desc).order(id: :asc).page(params[:page]).per Settings.paginate.hotels
    end
  end
